...
/**
 * This is mostly a copy of {@link com.intellij.openapi.keymap.impl.ui.ShortcutTextField}, which for
 * some reason has a package-private constructor; probably to force plugins to register shortcuts
 * for their actions globally.
 */
public final class ShortcutTextField extends ExtendableTextField {
    ...
    public ShortcutTextField(boolean isFocusTraversalKeysEnabled) {
    ...
    protected void processKeyEvent(KeyEvent e) {
        ...
        final boolean isNotModifierKey =
        ...
        if (isNotModifierKey) {
            // NOTE: when user presses 'Alt + Right' at Linux the IDE can receive next sequence
            // KeyEvents: ALT_PRESSED -> RIGHT_RELEASED ->  ALT_RELEASED
            ...
            if (e.getID() == KeyEvent.KEY_PRESSED) myLastPressedKeyCode = keyCode;
        }
    ...
    private Action getPopupAction(@NotNull KeyStroke stroke) {
        return new AbstractAction(KeymapUtil.getKeystrokeText(stroke)) {
            @Override
    ...
    private static void addKeyStrokes(
            @NotNull ArrayList<? super KeyStroke> list,
            @Nullable Iterable<? extends AWTKeyStroke> strokes) {
        if (strokes != null) {
            for (AWTKeyStroke stroke : strokes) {
                int keyCode = stroke.getKeyCode();
                if (keyCode != KeyEvent.VK_UNDEFINED) {
                    list.add(
                            stroke instanceof KeyStroke
                                    ? (KeyStroke) stroke
                                    : KeyStroke.getKeyStroke(
                                            keyCode,
                                            stroke.getModifiers(),
